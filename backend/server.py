...
async def get_dashboard_statistics(current_user: User = Depends(get_current_active_user)):
    # Get counts for main entities
    total_clients = await db.clients.count_documents({"archived": False})
    total_projects = await db.projects.count_documents({"archived": False})
    total_campaigns = await db.campaigns.count_documents({"archived": False})
    ...
    pending_invoices = len([invoice for invoice in invoices if invoice["status"] in ["sent", "overdue"]])
    
    # Get project statistics
    projects = await db.projects.find({"archived": False}).to_list(length=None)
    projects_by_status = {}
    ...
    recent_activities = [
        {
            "type": "project",
            "action": "created",
            "description": f"{len([p for p in projects if p['status'] == 'in_progress'])} projects in progress",
            "timestamp": datetime.utcnow().isoformat()
        },
        {
            "type": "invoice", 
            "action": "paid",
            "description": f"{len([i for i in invoices if i['status'] == 'paid'])} invoices paid",
            "timestamp": datetime.utcnow().isoformat()
        }
    ]
